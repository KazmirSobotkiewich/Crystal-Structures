# This code plots the electrostatic potential map for VO2 in the Rutile R phase.
...
Z = f.electrostatic_potential_R(X, Y, miller_indices)

## Plot the Electrostatic Potential Map for VO2 in the Rutile R phase
...
plt.title('Cross Section of $VO_2$ in $R$ Phase')
plt.xlabel('Along $a_R + b_R\:(\AA)$')
plt.ylabel('Along $c_R\:(\AA)$')
#plt.savefig('Cross-Section-VO2-R.png') # If we want to automatically save the figure
plt.figure()
plt.show()